...
- 답: Create a merge commit은 A, B, C의 커밋이 그대로 main 브랜치로 병합되고, Squash and merge는 A, B, C의 커밋이 하나의 커밋(squash)으로 main 브랜치로 병합되고, Rebase and merge는 A, B, C의 커밋을 모두 새로운 커밋으로 변경한다
...
- 답: revert. reset은 commit을 아예 삭제하는것이므로 위험하고 commit으 공유하는 다른 브랜치에도 영향을 줄 수 있다.
